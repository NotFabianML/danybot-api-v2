...
def gcode_file_rename(file_name="gcode_files/temp_routine.gcode"):
    ...
    folder = "gcode_files/"
    name = "routine_" + now_str + ".gcode"
    new_file_name = folder + name  # Nombre del nuevo archivo

    new_file_path = os.path.join(base_path, new_file_name)  # Construir la ruta completa

    try:
        os.rename(file_path, new_file_path)
        print (f"Gcode File Rename from \"{file_name}\" to \"{new_file_name}\"")

        #Contar la cantidad de lineas del archivo GCODE
    ...
    gcode_info = {"filename": name, "total_lines": total_lines, "filepath": new_file_name, "timestamp": now_timestamp }
    #print("GCODE INFO: " + str(gcode_info))
